(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.DjedSdk = {}, global.ethers));
  ...
  const getDjedContract = (providerOrSigner, DJED_ADDRESS) => {
    return new ethers.ethers.Contract(DJED_ADDRESS, djedArtifact.abi, providerOrSigner);
  };
  ...
  const getCoinContracts = async (djedContract, providerOrSigner) => {
    ...
    const stableCoin = new ethers.ethers.Contract(stableCoinAddress, coinArtifact.abi, providerOrSigner);
    const reserveCoin = new ethers.ethers.Contract(reserveCoinAddress, coinArtifact.abi, providerOrSigner);
  ...
  const getOracleContract = (providerOrSigner, oracleAddress) => {
    return new ethers.ethers.Contract(oracleAddress, oracleArtifact.abi, providerOrSigner);
  };
